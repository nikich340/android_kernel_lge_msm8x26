...
#define DRV_NAME "msm8226-asoc-tomtom"
					...
					bool dapm)
{
	...
	if (enable) {
		if (!codec_clk) {
			dev_err(codec->dev, "%s: did not get MCLK\n",
					__func__);
			ret = -EINVAL;
			goto exit;
		}
		...
		if (ret) {
			dev_err(codec->dev, "%s: clk_prepare failed, err:%d\n",
				__func__, ret);
			clk_users--;
			goto exit;
		}
		...
		if (clk_users > 0) {
			...
			dev_err(codec->dev, "%s: Error releasing codec MCLK\n", __func__);
			ret = -EINVAL;
			goto exit;
		}
	}
...
static int msm8226_wcd93xx_codec_up(struct snd_soc_codec *codec)
{
	int err;
...
static int msm8226_codec_event_cb(struct snd_soc_codec *codec,
	enum wcd9xxx_codec_event codec_event)
{
	switch (codec_event) {
	case WCD9XXX_CODEC_EVENT_CODEC_UP:
		return msm8226_wcd93xx_codec_up(codec);
	default:
...
static int msm_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	tomtom_event_register(msm8226_codec_event_cb, rtd->codec);
			     ...
			     struct snd_pcm_hw_params *params)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		/* For wcd9xxx_tx1 case */
		if (codec_dai->id == 1)
			user_set_tx_ch = msm_slim_0_tx_ch;
		/* For wcd9xxx_tx2 case */
...
static struct snd_soc_dai_link msm8226_9330_dai[] = {
	/* Backend DAI Links */
	{
...
static struct snd_soc_dai_link msm8226_9330_dai_links[
				ARRAY_SIZE(msm8226_common_dai) +
				ARRAY_SIZE(msm8226_9330_dai)];

static struct snd_soc_dai_link msm8226_9330_dai_links[
				ARRAY_SIZE(msm8226_common_dai) +
				ARRAY_SIZE(msm8226_9330_dai)];

struct snd_soc_card snd_soc_card_msm8226_wear = {
	.name		= "msm8226-tomtom-snd-card",
	.dai_link	= msm8226_9330_dai_links,
	.num_links	= ARRAY_SIZE(msm8226_9330_dai_links),
};
...
static int msm8226_prepare_codec_mclk(struct snd_soc_card *card)
{
	...
	if (pdata->mclk_gpio) {
		...
		if (ret) {
			dev_err(card->dev,
				"%s: Failed to request mclk gpio %d, err %d\n",
		...
		struct msm8226_asoc_mach_data *pdata)
{
	...
	if (pdata->us_euro_gpio < 0) {
		...
		rc = gpio_request(pdata->us_euro_gpio,
						  "TOMTOM_CODEC_US_EURO_GPIO");
		if (rc) {
			dev_err(&pdev->dev,
				"%s: Failed to request  us-euro gpio %d\n",
...
static const struct of_device_id msm8226_asoc_machine_of_match[]  = {
	{ .compatible = "qcom,msm8226-audio-tomtom", .data = "tomtom_codec"},
	{},
};
...
static struct snd_soc_card *populate_snd_card_dailinks(struct device *dev)
{
	...
	match = of_match_node(msm8226_asoc_machine_of_match, dev->of_node);
	if (!match) {
	...
	if (!strcmp(match->data, "tomtom_codec")) {
		card = &snd_soc_card_msm8226_wear;

		memcpy(msm8226_9330_dai_links, msm8226_common_dai,
				sizeof(msm8226_common_dai));
		memcpy(msm8226_9330_dai_links + ARRAY_SIZE(msm8226_common_dai),
			msm8226_9330_dai, sizeof(msm8226_9330_dai));
