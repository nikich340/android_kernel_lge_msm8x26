...
#if 1 // Test for E10 Panel Check sw84.lee
...
static int mxt_parse_dt(struct device *dev, struct mxt_platform_data *pdata)
{
	...
	TOUCH_INFO_MSG("DT : ghost_detection_card_enable: %d",pdata->ghost_detection_card_enable);
	...
	TOUCH_INFO_MSG("DT : ghost_detection_invalid_amplitude_enable: %d",pdata->ghost_detection_invalid_amplitude_enable);
...
static int mxt_write_config(struct mxt_fw_info *fw_info)
{
	...
	if (current_crc == fw_info->cfg_crc) { // || t38_cfg_crc == fw_info->cfg_crc) {
		...
		if ((data->panel_check_revB == 0x00) && (lge_get_board_revno() < HW_REV_A)) {
			ret = mxt_panel_write_t38(data);
			if (ret) {
				TOUCH_INFO_MSG("Failed rewrite Panel Status\n");
				return -EINVAL;
			}
		}
		return 0;
